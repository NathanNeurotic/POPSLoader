...
function PLDR.GetMassDriverName(index)
  ...
  if type(System) == "table" then
    ...
    if type(System.getMassDriverName) ~= "function" and type(System.getMassDriver) ~= "function" and type(System.getMassBackendInfo) == "function" then
...
local function BuildMassRootIdentity()
  ...
  for i = 1, #present do
    ...
    if driver ~= nil and string.find(driver, "sdc", 1, true) then
...
function PLDR.GetRootsByType(kind, mass_snapshot)
  ...
  if wanted == "usb" then
    for i = 1, #identity.usb do
      table.insert(roots, identity.usb[i])
    end
  elseif wanted == "mx4sio" then
    for i = 1, #identity.mx4sio do
      table.insert(roots, identity.mx4sio[i])
    end
  end
  return roots
end
