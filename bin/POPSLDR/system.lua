...
function PLDR.RunPOPStarterGame(gamelocation, game, ui_scene)
  ...
  if policy.name == "HDD" then
    ...
    if popstarter_on_hdd then
      ...
      hdd_init = EnsureHDDReadyForLaunch(selected_entry, nil, launch_slot)
      if not hdd_init.init_ok or hdd_init.status ~= 0 or not hdd_init.mount_ok then
        BlockLaunchFailure(
          "HDD launch prep failed",
          popstarter,
          device_page,
          nil,
          selected_entry,
          APP_DIR_LOCAL,
          hdd_init and hdd_init.status or nil,
          hdd_init and hdd_init.mount_partition or nil
        )
        return
      end
    end
  ...
  if popstarter_on_hdd then
    argv = {popstarter}
    if argv0_selector ~= nil and argv0_selector ~= "" and argv0_selector ~= popstarter then
      table.insert(argv, argv0_selector)
    end
