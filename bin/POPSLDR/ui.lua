        ...
        if UI.Pad.Events.BACK then
          ...
          elseif UI.MainMenu.OPT == 3 then
            ...
            if type(PLDR.ReadAtaProbeMarker) == "function" then
              ...
              if stuck ~= nil then
                -- The marker is now a single "in-flight" write (one MC write per
                -- entry, not ~25) -- it says a probe never RETURNED, and the
                -- photographed step number says where. Don't print the value.
                UI.Notif_queue.add("Last exFAT HDD check never finished\nplease photograph the step number this time", "warn")
              end
            end
