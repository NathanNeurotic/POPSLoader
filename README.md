...
| `<device>:/ART/GameName_COV.png` | Optional cover art (200x200 8-bit PNG recommended), named with the **OPL `_COV.png` convention** so an existing OPL `ART` folder works as-is. Covers are read from **exactly one** folder — your choice in *Settings → Game List → Cover/details folder*: **device:/ART** (default; a top-level `ART/` at the device root, matching OPL's `mass:/ART`), **device:/POPS/ART** (an `ART/` subfolder), or **device:/POPS** (beside the game). The matching `GameName.txt` *Game details* sidecar rides the same folder. |
...
| **Game details** | **Off** (default) · Left · Center · Right | Shows a per-game blurb from a `<game>.txt` sidecar in a small panel under the cover, in the chosen text alignment (*Off* hides it). The `.txt` is sought in the **single** folder *Cover/details folder* is set to (a top-level `ART/` by default), and the disc-marker-stripped name is tried first so one file serves every disc of a multi-disc game. Authored line breaks are preserved; scroll a long blurb with the **right analog stick**. |
| **Cover/details folder** | **ART (at root)** (default) · POPS/ART · POPS (beside game) | Picks the **single** folder where each game's cover (`<name>_COV.png`, the OPL convention) and its `<name>.txt` details sidecar are read on removable devices (USB / MX4SIO / MMCE): a top-level `<device>:/ART/` (default, matches OPL's `mass:/ART`), a `POPS/ART/` subfolder, or the game's own `POPS/` folder beside the `.VCD`. **Only the chosen folder is read** — there is no second fallback location. The folder is listed once and cached, so a missing cover never stalls the browser. The internal HDD keeps its fixed `__common/POPS/ART/` layout. |
...
*   Place the cover in the **single** folder chosen by *Settings → Game List → Cover/details folder* (default a top-level **ART** at the device root, or **POPS/ART**, or **POPS** beside the game). Only the chosen folder is read — a cover in a different folder won't show. On the internal HDD covers live in `__common/POPS/ART/`.
