...
- HDD-backed `POPSTARTER.ELF` handoff (`D-10`, `D-14`)
  ...
  - current source now keeps the confirmed HDD startup auto-init and USB first-entry fixes, preserves the restored `D-15` split, keeps the HDD-backed reboot handoff on mounted `pfs0:/...`, passes the original HDD source context into the embedded loader explicitly, resets IOP there only when that explicit source context is HDD-backed, fixes the malformed direct-HDD alias builder, and aligns the parent-to-loader ownership more closely to `wLaunchELF` by using `SifExitRpc()` before the embedded-loader `ExecPS2`, skipping pre-handoff PFS unmounting there, and calling `SifLoadElf` in the embedded loader without `SifLoadFileInit/Exit`.
