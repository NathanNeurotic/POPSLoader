...
- HDD-backed `POPSTARTER.ELF` handoff (`D-10`, `D-14`)
  ...
  - current repo line now restores partition context only as loader metadata for HDD-backed POPSTARTER so the child uses the remount/reset path again, while still keeping no launch CWD and the same visible resolved exec path.
