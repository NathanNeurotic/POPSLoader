...
- HDD-backed `POPSTARTER.ELF` handoff (`D-10`, `D-14`)
  ...
  - current repo line keeps the partition-aware HDD reboot contract, separate exec-path reporting, profile-path normalization, and now moves to a bounded child-loader fallback handoff for partition-aware HDD POPSTARTER loads.
  ...
  - audit then found one more regression in the exact HDD child-loader path now in use: older child-loader source reloaded `SIO2MAN`, `MCMAN`, and `MCSERV` after HDD `SifIopReset("")`, while later `HEAD` had drifted to jump straight to `ExecPS2`. After the no-reset-first + fileXio-first iteration still black-screened, current source now takes the bounded fallback iteration for partition-aware HDD POPSTARTER: mount `pfs0:`, load via mounted-path `SifLoadElf`, then apply child-side `SifIopReset("")` + `SIO2MAN`/`MCMAN`/`MCSERV` reload before `ExecPS2`. Hardware on this exact line is still `Unknown (verify on hardware)`.
...
- `D-10` HDD POPSTARTER on HDD:
  ...
  - current repo line keeps the partition-aware reboot contract, separate exec-path reporting, and profile-path normalization, and now uses the bounded fallback child-loader handoff for partition-aware HDD POPSTARTER loads.
  - latest user hardware report from the 2026-04-02 GitHub artifact (`cd76569`) still black-screened.
  - after the no-reset-first + mounted fileXio-first iteration still black-screened, current source now uses the bounded fallback partition-aware child-loader sequence: mount `pfs0:`, load via mounted-path `SifLoadElf`, then do child-side `SifIopReset("")` + `SIO2MAN`/`MCMAN`/`MCSERV` reload before `ExecPS2`; exact-line hardware status is `Unknown (verify on hardware)`.
  - a later hardware re-test on that child-remount/cold-parent line still black-screened.
  - the later stripped direct-HDD-ELF line moved `D-10` back to a returned `rc=-1`, but the popup also showed the launcher was still probing/opening `pfs3:/.../POPSTARTER.ELF` while separately trying to exec a rewritten `pfs:/.../POPSTARTER.ELF`.
...
- `D-14` HDD-backed POPSTARTER with non-HDD game:
  ...
  - current repo line uses the same partition-aware HDD reboot contract as `D-10`, including the bounded fallback child-loader sequence (`pfs0:` remount, mounted-path `SifLoadElf`, child-side reset/module reload, then `ExecPS2`); a current-line hardware re-test is still `Unknown (verify on hardware)`.
