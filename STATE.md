...
| **BOOT.ELF from HDD-booted POPSLoader** (U-10) | **FAIL — known broken accepted** | 2026-05-28 PM late (Nuno, after a more thorough sweep) | Earlier "across the board PASS" report was actually the non-HDD subset Nuno tested in that round. After explicit HDD-booted retests: HOSDmenu → HDD POPSloader → exit BOOT.ELF = black screen; wLE → HDD POPSloader → exit BOOT.ELF = black screen. Other devices (MC, USB, MX4SIO) → BOOT.ELF exit OK. So U-10 is unchanged from prior BETA-10-5 status — pragmatically accepted, workaround: Exit → OSDSYS or reboot. Maintainer hypothesis: "maybe I need to reset IOP on boot before anything else" (aligns with H1 dev9Shutdown / H5 stale pfs1: mount in `docs/U10_INVESTIGATION.md`). |
...
- **U-10 BOOT.ELF-from-HDD-boot** — when POPSLoader was launched from HDD (via HOSDmenu, wLE, etc.), Exit → BOOT.ELF black-screens. Long-standing; the earlier 2026-05-28 PM "across the board PASS" report turned out to cover only the non-HDD-booted cases Nuno had tested in that round; an explicit HDD-booted retest re-confirmed the failure. Other launch contexts → BOOT.ELF still work. Workaround: Exit → OSDSYS or reboot. Maintainer hypothesis to investigate next: explicit IOP reset on boot before anything else (aligns with `docs/U10_INVESTIGATION.md` H1/H5).
