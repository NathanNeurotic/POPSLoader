...
- `D-10` HDD POPSTARTER on HDD:
  ...
  - current source therefore restores partition context only as loader metadata for HDD-backed POPSTARTER so the child uses the remount/reset path again, while still keeping zero selector/game args, no launch CWD, and the same visible resolved exec path.
  - audit also found a Lua binding bug in `src/luasystem.cpp`: the trailing reboot partition context was only parsed when `System.loadELF(...)` had at least four Lua arguments, so the new zero-arg HDD path could not actually deliver loader-only partition metadata. Current source now accepts that trailing partition context with three arguments as well.
