...
- `D-10` HDD POPSTARTER on HDD:
  ...
  - current source explicitly unmounts `pfs0:` and skips `SifExitRpc` right before executing an HDD-backed target ELF, to prevent `-EBUSY` block collision while leaving SIF alive for POPSTARTER. Hardware status is `Unknown (verify on hardware)`.
