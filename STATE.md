...
**Open:**
- **SMB fails to connect on real hardware even with correct settings** (maintainer, 2026-07-06) — root cause identified from source by the 2026-07-07 audit: `EnsureNet` initializes the EE-side lwip stack while `smbman` uses the IOP-side stack, so IP config/DHCP never reach the stack that matters (fix = OPL's `ps2ip_init()` RPC bind; queued, needs HW retest). The full audit — 62 verified findings incl. contradictory settings, silently-discarded settings, cwd-default gaps, and a launch-failure-diagnostic crash — lives in **[docs/REPO_AUDIT_2026-07-07.md](docs/REPO_AUDIT_2026-07-07.md)**. Note: the SMB Port default **1111 is deliberate** (maintainer's PS2-Servers custom-port ecosystem), not a bug.
