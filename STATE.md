...
- `D-10` HDD POPSTARTER on HDD:
  ...
  - current source now removes that stale exec-path rewrite from the stripped HDD-backed POPSTARTER experiment, so probe/open and exec both use the same resolved HDD ELF path.
