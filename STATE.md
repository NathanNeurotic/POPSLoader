...
- Current CI package also includes `PS1_POPSLOADER/BUILD_INFO.txt`, and the workflow fails if the built ELF is missing key embedded runtime markers or if a touch of `src/elf_loader/src/loader/src/loader.c` followed by a clean rebuild does not regenerate the full embedded-loader chain (`loader.elf` -> generated blob -> `libcustom-elf-loader.a` -> `enceladus.elf` -> `POPSLOADER.ELF`).
...
- `D-10` HDD POPSTARTER on HDD:
  ...
  - current source first tried a no-reset-first child-loader handoff for partition-aware HDD POPSTARTER with mounted-path fileXio-first and mounted-path `SifLoadElf` fallback, but that line still black-screened on hardware.
  - current source now takes the bounded fallback iteration for this partition-aware child path: mount `pfs0:`, load through mounted-path `SifLoadElf`, then apply child-side `SifIopReset("")` + `SIO2MAN`/`MCMAN`/`MCSERV` reload before `ExecPS2`; hardware on this exact line is `Unknown (verify on hardware)`.
  - latest user hardware report from the 2026-04-02 GitHub artifact (`cd76569`) still black-screened on `D-10`.
  - latest user hardware report from the 2026-04-02 GitHub artifact after the mounted fileXio-first child-loader iteration (`0565ae5`) also still black-screened on `D-10`.
...
- `D-14` HDD-backed POPSTARTER with non-HDD game:
  ...
  - a later 2026-03-28 re-test on the direct-`hdd0:PART:pfsN:/POPSTARTER.ELF` preference source still black-screened on `X`.
  - current source now uses the same partition-aware HDD reboot contract as `D-10`, including the bounded fallback child-loader sequence (`pfs0:` remount, mounted-path `SifLoadElf`, child-side reset/module reload, then `ExecPS2`), rather than the earlier ad hoc source-context handoff or whichever mounted `pfsN:` path Lua happened to resolve first.
...
- `U-10` BOOT.ELF after HDD page init:
  ...
  - current source now keeps the no-launch-CWD rollback, keeps standard external-launch prep, retries `reboot_iop = 1` for BOOT.ELF after HDD init, and restores the generic reboot path in `src/elf_loader/src/elf.c` to the repo's older embedded-loader handoff style after the post-reset cleanup/module-reload contract from `src/system.cpp`.
