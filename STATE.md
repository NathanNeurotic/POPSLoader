...
- A live cover is a `<name>.png` sidecar; it draws into its own `COVER_W` inset. `BuildCoverCandidates` seeks it the **same way on every device** (shipped `83e81cf`): a **`POPS/ART/` subfolder is tried FIRST** (the canonical, device-uniform location), then **beside the `.VCD`** as back-compat for art that predates the `ART/` folder. On removable devices that is `<device>:/POPS/ART/<game>.png` then `<device>:/POPS/<game>.png`; on HDD/PFS it is `hdd0:__common/POPS/ART/<game>.png` (mounted from the `__common` partition). Within each location the **disc-marker-stripped name is tried first** (so one art file serves every disc of a multi-disc game), then the exact per-disc `<full-name>.png`. The matching **`<name>.txt` details sidecar rides the same candidate list** (each `.png` → `.txt`).
