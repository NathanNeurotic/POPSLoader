...
/*  POPSLoader: the tail of the ladder below is thirty 1-second sleeps, so one
    wait can burn ~31s before it gives up. A PS2's internal ATA bus has a master
    only -- there is no slave drive to answer -- yet ata_init_devices() still
    soft-resets and selects device 1, and each of those waits runs the ladder to
    the very end. Several ~31s waits in a row is the multi-minute stall testers
    photograph as a frozen exFAT page (sAGA, 4TB; maintainer, ~5 min then OK).

    Cap the ladder at 100ms/step ONLY while probing the phantom slave: worst case
    ~4s instead of ~31s. Device 0 keeps the full ladder untouched -- a cold drive
    can legitimately need seconds to spin up, and shortening THAT would trade a
    slow page for an undetected disk. */
static int ata_probing_phantom_slave = 0;
...
static int gen_ata_wait_busy(int bits)
{
    ...
    for (i = 0; i < 80; i++) {
        ...
        if (ata_probing_phantom_slave && delay > 100000)
            delay = 100000;
...
static int ata_init_devices(ata_devinfo_t *devinfo)
{
    ...
    if (!ata_dvrp_workaround) {
        for (i = 0; i < 2; i++) {
            /* Device 1 is the slave. A retail PS2's ATA bus has no slave, so
               every wait below runs the busy ladder to its full ~31s. Cap it. */
            ata_probing_phantom_slave = (i == 1);

            if ((res = sceAtaSoftReset()) != 0) {
                ata_probing_phantom_slave = 0;
                return res;
            }
