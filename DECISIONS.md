...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - current source therefore keeps the restored non-HDD POPSTARTER path, keeps the HDD-backed reboot handoff on mounted `pfs0:/...`, removes EE-side SIF teardown before `ExecPS2` into the embedded loader, and additionally stops preserving boot PFS slots during launch prep for HDD-backed `POPSTARTER.ELF`.
