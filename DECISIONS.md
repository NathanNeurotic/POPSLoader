...
- HDD startup auto-init on HDD boot/configured paths:
  ...
  - repo inspection of `etc/boot.lua` also showed HDD boot is established on a dedicated `pfs1:` mount before `system.lua` runs, so current source now carries that exact boot partition/slot metadata into `system.lua`, seeds the HDD mount tracker from it, and rebuilds raw HDD sidecar/source-context paths from mounted `pfs1:` candidates instead of assuming low-level HDD readiness alone preserves the boot-side path contract.
