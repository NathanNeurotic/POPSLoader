...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - a later 2026-03-30 hardware re-test restoring `fileXioInit` still returned to OSDSYS. Deep code review then exposed the final logic flaw: the parent loader (`elf.c`) was indeed instructed to *preserve* `pfs0:` during teardown (using `keep_mask_out = 1`), but it never actively mounted the required HDD partition to `pfs0:` in the first place, meaning `fileXioOpen("pfs0:/POPSTARTER.ELF")` inevitably failed. The previous embedded loader codebase included the mount step internally, but since we removed it to match `wLaunchELF`, we must now explicitly mirror `wLaunchELF`'s parent execution path, which mounts the partition immediately prior to jumping into the child loader.
