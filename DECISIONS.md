...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - current source then realigned that partition-aware HDD child handoff to a no-reset-first path with mounted fileXio-first plus mounted `SifLoadElf` fallback, but that line still black-screened on hardware.
  ...
  - current source now moves to the bounded fallback iteration for partition-aware HDD POPSTARTER in the child loader: mount `pfs0:`, load via mounted-path `SifLoadElf`, then apply child-side `SifIopReset("")` + `SIO2MAN`/`MCMAN`/`MCSERV` reload before `ExecPS2`. Hardware on this exact line remains `Unknown (verify on hardware)`.
