...
- Evidence: `bin/POPSLDR/system.lua` (`PLDR.HDD.EnsureBootPartitionWritable`, `ComputeSettingsSidecarPath`, HDD `.hide` write path), `bin/POPSLDR/ui.lua` (L3 hide binding + handler; R3 is bound but currently has no handler). provato hardware confirmation 2026-06 (HDD RW-writable). See STATE.md > "Settings (single-device parity)" and "Per-game hide layer".
