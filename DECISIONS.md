...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - current source therefore keeps the restored non-HDD POPSTARTER path, keeps the HDD-backed reboot handoff on mounted `pfs0:/...`, removes EE-side SIF teardown before `ExecPS2` into the embedded loader, stops preserving boot PFS slots during launch prep for HDD-backed `POPSTARTER.ELF`, passes the original HDD source context into the embedded loader explicitly, resets IOP there only when that explicit source context is HDD-backed, and fixes the malformed direct-HDD alias builder.
