...
- HDD startup auto-init on HDD boot/configured paths:
  ...
  - repo inspection of `etc/boot.lua` also showed HDD boot is established on a dedicated `pfs1:` mount before `system.lua` runs, so current source now recreates that boot mount after startup HDD init instead of assuming low-level HDD readiness alone preserves the boot-side path contract.
