...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - 2026-05-20 simplification pass addresses the over-engineering risk: normal HDD-backed POPSTARTER launches no longer use Lua partition context, generic `pfs:/...` exec rewriting, the Lua HDD pre-exec gate, or the mounted-PFS fallback remount path by default. They keep the resolved executable path and call legacy `System.loadELF(path, reboot_iop, selector)`.
