...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - current source therefore restores partition context only as loader metadata for HDD-backed POPSTARTER so the child uses the remount/reset path again, while still keeping no selector arg, no game arg, no launch CWD, and the same visible resolved exec path.
  ...
  - current working rule from repo evidence: POPSTARTER itself does not need slot preservation, launch CWD, partition context, or other carried runtime state after exec. The current temporary target is only to make the HDD-backed ELF launch successfully at all; selector/`argv[0]` restoration comes afterward. The loader may still need partition metadata to load the HDD ELF cleanly.
