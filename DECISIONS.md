...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - current source now explicitly restores `SifIopReset` in the embedded loader strictly *after* `fileXioOpen` has successfully parsed and loaded the ELF into EE RAM, ensuring POPSTARTER receives its required sterile IOP execution environment.
