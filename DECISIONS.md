...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - a later 2026-03-30 hardware re-test testing the memory fix still returned to OSDSYS. Deep code review revealed that the "Profile 1/Default" sidecar launch (e.g. `POPSTARTER.ELF`) failed `is_hdd_backed_exec_path()` because it lacked an explicit `pfs0:` prefix. This failure unmasked why the user saw 3 destructive IOP resets: because the sidecar path evaluated to false, the system triggered a destructive legacy standard external launch rather than utilizing our custom embedded retention pathways.
  - a later 2026-03-30 hardware re-test fixing the relative CWD path fallback still returned to OSDSYS. This execution confirmed the embedded loader was finally correctly hit (3 background flashes), but `fileXioOpen` failed to read the ELF. A code comparison with `wLaunchELF` showed that `wLaunchELF` does NOT call `fileXioInit()` within the child loader, relying entirely on `SifInitRpc(0)`. Re-initializing fileXio in the child likely reset the existing RPC context, terminating the inherited mount and failing the `open()` call.
  - a later 2026-03-30 hardware re-test removing `fileXioInit` still returned to OSDSYS. Further review confirmed that the `ExecPS2` jump restarts the EE kernel, making `fileXioInit` absolutely mandatory to bind the RPC client back to the active IOP `fileXio` module. The previous hypothesis that `fileXioInit` was destructive was false; the destruction was caused by the parent's `SifIopReset`. The current code restores `fileXioInit` inside the embedded loader to properly communicate with the retained `pfs0:` partition.
  - a later 2026-03-30 hardware re-test restoring `fileXioInit` and explicitly mounting `pfs0:` returned `-1` after 21s instead of OSDSYS! This execution confirmed the child loader successfully bound RPC and attempted `fileXioOpen`, but the open timed out. Code review revealed Lua was not generating the `hdd0:` partition context name for relative sidecar paths (`POPSTARTER.ELF`). Without the context, the parent passed the `pfs3:/...` path directly to the child, which unmounted the partition during teardown instead of remounting it to `pfs0:`.
  - a later 2026-03-30 hardware re-test checking the Lua relative path fallback still returned to OSDSYS. Code review exposed that `System.getAppDir()` boot partitions were completely bypassing `PLDR.HDD.mounted_slots` population, so Lua returned `nil` for the partition context, reverting the logic to the faulty unmounted `pfs3:/...` handoff. Current source fixes Lua's `ResolvePopstarterPartitionContext` to explicitly extract the `hdd0:` prefix from the APP_DIR fallback string if the slot array fails, explicitly forcing the correct C++ parent loader `pfs0:` remount block.
  - a later 2026-03-30 hardware re-test executing the properly remounted `pfs0:` context still returned to OSDSYS. Deep code review confirmed the execution correctly routed to the embedded loader, but the final `fileXioOpen` timed out. A direct comparison with `wLaunchELF` showed that `wLaunchELF` explicitly passes full permission mode bits (`0777`) to `fileXioOpen` on PFS. Passing `0` caused the IOP `ps2fs` module to silently reject the file descriptor request. The current codebase fixes this final open failure by applying the exact `0777` flag bits used by `wLaunchELF`.
  - a later 2026-03-30 hardware re-test evaluating the complete pipeline revealed a final sequence flaw: execution instantly failed and returned `rc=-1` from C++ to the launcher menu without ever executing the embedded loader! Code tracing confirmed the parent loader's newly added `fileXioMount` operation instantly failed because the target partition was already mounted on `pfs3:` by the background scanner. The PS2 strictly prohibits duplicate block device mounts on PFS. Current code moves the parent unmount routine *before* the `pfs0:` remount to explicitly free the target partition block, completing the true `wLaunchELF` flow mapping.
  - a later 2026-03-30 hardware re-test finally resolved the OSDSYS timeout loop and entered the `POPSTARTER.ELF` handoff, but resulted in a black screen! Deep review proved that bypassing the IOP reset in the embedded loader (to mirror `wLaunchELF`) successfully allowed `fileXioOpen` to parse the ELF, but it left the heavy `ps2fs` modules running in IOP RAM. `POPSTARTER` requires a completely pristine, wiped IOP state seeded with Memory Card drivers (`MCMAN/MCSERV`) before `ExecPS2`. Bypassing the IOP reset predictably crashed the commercial engine wrapper on startup.
  - a later 2026-03-31 hardware re-test restoring `SifIopReset` to scrub the IOP resulted in another black screen. Further code review identified that resetting the IOP wiped the RPC command servers needed by `SifExitRpc()`. This meant the EE SIF DMA was left infinitely blocking on the teardown commands prior to executing `ExecPS2`, never actually starting the game engine.
  - current source explicitly removes the `SifIopReset` block entirely from the embedded loader (`loader.c`). This gracefully mirrors `wLaunchELF`'s true execution flow, maintaining the exact active IOP state (and active `pfs0:` mount) that `fileXio` used to successfully read the ELF, without hanging the EE's RPC teardowns.
