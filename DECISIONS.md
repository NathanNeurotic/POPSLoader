...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - a new code analysis confirmed that while the embedded loader mounts `pfs0:` for HDD-backed POPSTARTER via `fileXio`, it never unmounted it before executing `ExecPS2`. Furthermore, the non-fileXio path still incorrectly terminated SIF interfaces. The current source corrects this by unmounting `pfs0:` immediately before exec and ensuring `SifExitRpc` is skipped so POPSTARTER inherits the required IOP environment.
