...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - current source then realigns that partition-aware HDD child handoff to a no-reset-first path: mount `pfs0:` + `SifLoadElf`, then `SifExitRpc`/`ExecPS2` without a second child-side `SifIopReset("")` + module-reload stage. Hardware on this exact line remains `Unknown (verify on hardware)`.
