...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - the user then explicitly narrowed the short-term objective further: selector correctness and partition-context handoff are no longer the immediate targets; first prove that an HDD-backed `POPSTARTER.ELF` can start at all.
  - current source therefore bypasses the partition-aware HDD handoff entirely for this specific startup experiment, still keeps the safer embedded-loader fix (`e2c4b8f`) that avoids `printf`/`snprintf` dependence in that environment, returns the actual embedded-loader `ExecPS2` result instead of collapsing it to `-1`, executes the resolved HDD ELF with zero selector/extra args, derives the partition-scoped exec filename from the final resolved POPSTARTER path at launch time so stale precomputed values cannot drop the ELF basename when that path is used, normalizes stale canonical profile-path state before launch/save, keeps the older iomanX-aware `fileXio` path only for direct `pfs:` / `hdd:` loads with no partition-aware HDD context, and still exposes the real exec filename separately from the probe/open path in the launcher popup.
  - current working rule from repo evidence: POPSTARTER itself does not need slot preservation, launch CWD, partition context, or other carried runtime state after exec. The current temporary target is only to make the HDD-backed ELF launch successfully at all; selector/`argv[0]` restoration comes afterward.
...
- `BOOT.ELF` after HDD page init:
  ...
  - current source therefore keeps the no-launch-CWD rollback and standard external-launch prep, but retries `reboot_iop = 1` after HDD init as the next bounded BOOT.ELF variable.
