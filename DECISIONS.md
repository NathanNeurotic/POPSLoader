...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - the next 2026-03-28 hardware result on that partition-aware source no longer black-screened, but the launcher regained control with `rc=-1 (returned after 3528 ms)`, which means the parent `ExecPS2` jump into the embedded loader is returning instead of handing off cleanly.
  ...
  - current source restores that original parent-side jump contract while still keeping the safer embedded-loader fix (`e2c4b8f`) that avoids `printf`/`snprintf` dependence in that environment, returns the actual embedded-loader `ExecPS2` result instead of collapsing it to `-1`, and fixes `System.loadELF(path, reboot_iop, args...)` so it no longer drops every extra arg after the first one.
