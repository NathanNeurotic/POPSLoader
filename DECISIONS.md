...
- HDD `POPSTARTER.ELF` when launcher/sidecar/CWD is on HDD:
  ...
  - comparison against `wLaunchELF` and `PlayStation2-Basic-BootLoader-Extended` also showed another remaining mismatch: this repo's embedded loader was still unconditionally resetting IOP after `SifLoadElf`, while the reference loaders either make that reset conditional on explicit HDD context or default it off.
  - current source therefore keeps the restored non-HDD POPSTARTER path, keeps the HDD-backed reboot handoff on mounted `pfs0:/...`, removes EE-side SIF teardown before `ExecPS2` into the embedded loader, stops preserving boot PFS slots during launch prep for HDD-backed `POPSTARTER.ELF`, no longer unconditionally resets IOP inside the embedded loader before `ExecPS2`, and fixes the malformed direct-HDD alias builder.
