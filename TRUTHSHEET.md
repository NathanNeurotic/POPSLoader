...
- `BOOT.ELF from HDD-launched POPSLoader` (U-10): **FAIL** 2026-05-28 PM late (Nuno). When POPSLoader has been successfully launched from HDD (any working launcher), Exit → BOOT.ELF black-screens. MC/USB/MX4SIO-launched POPSLoader → BOOT.ELF exit still works. Workaround: Exit → OSDSYS or reboot. Maintainer hypothesis to investigate next: explicit IOP reset on boot before anything else (aligns with H1/H5 in `docs/U10_INVESTIGATION.md`).
- `HOSDmenu → POPSLoader` (Class A: POPSLoader fails to start): **FAIL** 2026-05-28 PM late (Nuno). HOSDmenu attempts to launch POPSLoader and the screen goes black before the POPSLoader splash — distinct from U-10, which is about exit FROM a running POPSLoader. Same launcher-IOP-state class PR #458 Layer A targeted, but Layer A's `fileXio` teardown didn't resolve it for HOSDmenu. Workaround: use a different launcher.
- `Some wLaunchELF builds → POPSLoader` (Class A: POPSLoader fails to start): **FAIL on some wLE builds** 2026-05-28 PM late (Nuno) + CosmicScale 2026-05-25. Common wLE builds work after PR #458 Layer A; specific builds still black-screen attempting POPSLoader launch. Workaround: switch wLE build, or use a different launcher.
