...
| 2026-05-28 PM (follow-up) | Nuno on rolling-release | **NEW failure** isolated after the broader PASS: when POPSLoader is launched from `mx4sio:/APPS/PS1_POPSLOADER/POPSLOADER.ELF`, settings save fails with "mx4sio:/APPS/PS1_POPSLOADER/.pldrs may be read-only". Screenshot captured. | Settings save MX4SIO-rooted | **FAIL** (Nuno 2026-05-28 PM hardware). Root cause: `mx4sio:/` argv0 prefix is the BDM device-kind label, not a writable fileXio mount. The launcher reads the ELF through kernel-side indirection but fileXio writes against `mx4sio:/<path>` fail. The writable filesystem path is the `mass*:/` slot where `bdmfs_fatfs` mounts the SD card. **Fix in flight: PR #476** `claude/mx4sio-boot-path-normalize` adds an `mx4sio:` branch to `etc/boot.lua` (mirrors the existing `hdd0:` branch): loads `usbmass_bd` + `mx4sio_bd`, sleeps for double-ping settle, scans `mass:/`..`mass9:/` for the slot whose ioctl driver name contains `sdc`/`mx4`, and rewrites cwd. `ResolveAppDirLocal` then prefers the translated `mass*:/` cwd over the raw `mx4sio:/`-prefixed APP_DIR so the settings sidecar resolves to a writable path. Test plan: hardware-verify on MX4SIO-rooted boot that save works; regression on USB/MC/MMCE/HDD saves; preservation contracts (D-10/D-14/D-15, DKWDRV-MC, BOOT.ELF) still PASS. |
