...
| 2026-05-28 | Open work | PR #471 (DRAFT) `claude/curious-noether-3f2a8`: Layer C `mmceman.irx` lazy-loaded unless `boot_device_hint == "MMCE"`. Includes the PR #473 hotfix and a MMCE/MC clarification commit. | Layer C mmceman | Repo / CI verified. Hardware UNKNOWN (DRAFT explicitly requires hardware verification). Critical regression checks: pad input survives on USB / HDD / MC boots; MMCE page entry from deferred state correctly lazy-loads. |
