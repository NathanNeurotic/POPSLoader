...
| 2026-04-19 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game on current tip `0565ae5` mounted fileXio-first HDD child-loader fallback | D-10 | FAIL: black screen |
| 2026-04-19 | Unknown (verify on hardware) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game on post-load reset HDD child handoff source | D-10 | Unknown (verify on hardware) |
...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - current source now restores the post-load reset child-loader handoff for partition-aware HDD POPSTARTER: mount `pfs0:`, load with mounted-path `SifLoadElf`, run child-side `SifIopReset("")`/`SifIopSync()`, reinitialize RPC, reload `rom0:SIO2MAN`, `rom0:MCMAN`, and `rom0:MCSERV`, then jump via `ExecPS2`. Hardware on this exact line is still `Unknown (verify on hardware)`.
  ...
  - `D-14`: reported FAIL on 2026-03-27 when launching a USB game with Profile 2 pointing `POPSTARTER.ELF` to HDD.
    ...
    - a later 2026-03-28 re-test on the direct-`hdd0:PART:pfsN:/POPSTARTER.ELF` preference source still black-screened on `X`.
    - current source now uses the same partition-aware HDD reboot contract as `D-10`, including the post-load reset child-loader handoff with mounted-path `SifLoadElf` plus child-side IOP reset/module reload, rather than the earlier ad hoc source-context handoff or whichever mounted `pfsN:` path Lua happened to resolve first; hardware result is still `Unknown (verify on hardware)`.
