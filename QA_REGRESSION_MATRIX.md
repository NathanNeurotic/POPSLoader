...
| 2026-03-30 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game with CWD resolution fix | D-10 | FAIL: OSDSYS fallback (Embedded loader executes but fileXioOpen fails) |
| 2026-03-30 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game with fileXioInit restored and .ELF extension | D-10 | FAIL: rc=-1 returned after 21s (Timeout on pfs3:) |
| 2026-03-30 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game with fileXioOpen mode bits 0777 | D-10 | FAIL: instant rc=-1 (fileXioMount returns -EBUSY) |
| 2026-03-30 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game with explicit secondary pfs slot unmount | D-10 | FAIL: black screen (ExecPS2 executed dirty IOP) |
...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - on that contract, the parent passes exact HDD partition context separately from the mounted load path and normalizes the partition-aware exec filename to generic `pfs:/...`.
    ...
    - a later 2026-03-30 hardware re-test fixing the memory wipe still returned to OSDSYS. Deep code review revealed that the "Profile 1/Default" sidecar path uses the relative filename `POPSTARTER.ELF`. The parent loader failed to resolve this relative path to its absolute `pfs0:/POPSTARTER.ELF` equivalent, meaning it failed the `is_hdd_backed_exec_path()` check. Because it wasn't flagged as an HDD execution, the parent performed a standard destructive `SifIopReset` fallback execution, entirely bypassing the specialized retention fixes!
    - a later 2026-03-30 hardware re-test solving the relative path fallback still returned to OSDSYS, but with ~3 distinct screen color flashes, proving the embedded loader executed but failed at `fileXioOpen`. Code review confirmed `loader.c` calls `fileXioInit()`, which rebinds RPC and likely corrupts the file descriptor mount established by the parent. `wLaunchELF` avoids calling `fileXioInit` and relies only on `SifInitRpc(0)`.
    - a later 2026-03-30 hardware re-test removing `fileXioInit` STILL returned to OSDSYS. Deep code review then verified that `ExecPS2` explicitly destroys the EE execution kernel context. Since `fileXioOpen` is an EE wrapper that fires an RPC request to the IOP module, it *must* re-initialize its local SIF client by calling `fileXioInit()`. If it fails to do so, the RPC request is sent over an unbound port, causing an immediate failure. The previous deduction that `fileXioInit` destroyed the mount was false; the destruction was caused by the parent's `SifIopReset`.
    - a later 2026-03-30 hardware re-test restoring `fileXioInit()` and explicitly mounting `pfs0:` STILL failed, but returned `rc=-1` after 21 seconds instead of OSDSYS! This proves the embedded loader successfully executed, bound its RPC, and called `fileXioOpen`, but the open operation timed out. Code review revealed that Lua was failing to pass the underlying HDD partition name (`hdd0:+OPL`) to C++ because `POPSTARTER.ELF` is a relative path. Without the partition name, the C++ parent skipped the `pfs0:` remount routine and passed the raw unmounted `pfs3:/...` path to the embedded loader, which timed out waiting for the non-existent mount.
    - a later 2026-03-30 hardware re-test attempting to resolve the relative CWD path fallback in Lua still returned to OSDSYS. Deep code review then exposed that `pfs3:` sidecar boot partitions were not populating `PLDR.HDD.mounted_slots`, meaning Lua's partition context resolver was returning `nil`. This returned the parent execution path to the faulty `pfs3:/...` load state without a partition name, which bypassed the `pfs0:` remount. Current code explicitly adds a `System.getAppDir()` fallback in `ResolvePopstarterPartitionContext` to extract the boot partition directly from the app dir string (`hdd0:...`), guaranteeing the `pfs0:` remount correctly fires in C++.
    - a later 2026-03-30 hardware re-test fixing the relative CWD path resolution returned to OSDSYS. With the parent properly executing the `pfs0:` remount block, the embedded loader was finally correctly passed `"pfs0:/POPSTARTER.ELF"`. The remaining failure was explicitly traced to `fileXioOpen` being called with `0` for the `mode` parameter. `wLaunchELF` specifically requires passing `0777` permissions when opening an ELF from PFS. The `ps2fs` IOP module silently rejects unprivileged handle requests, causing the timeout.
    - a later 2026-03-30 hardware re-test applying the 0777 permissions and Lua absolute path resolution still returned an instant `rc=-1` failure. Deep review of the execution trace verified the parent loader attempted to explicitly execute `fileXioMount("pfs0:", ...)` as requested, but failed instantly because the target partition was already mounted by Lua to the background scanner's `pfs3:` slot! The PS2 PFS strictly blocks concurrent block device mounts, returning `-EBUSY` instantly and aborting the launch before the embedded loader even runs.
    - a later 2026-03-30 hardware re-test fixing the unmount priority successfully loaded the ELF from `pfs0:`! However, after bypassing the execution timeout and OSDSYS failures, the handoff to POPSTARTER resulted in a black screen. Deep review of the embedded loader (`loader.c`) confirmed that we had previously completely removed `SifIopReset` to mirror `wLaunchELF`. Unlike homebrew ELFs, `POPSTARTER.ELF` is a heavily customized engine that strictly expects the IOP memory to be entirely scrubbed and seeded with `SIO2MAN`, `MCMAN`, and `MCSERV` before `ExecPS2`. Bypassing the IOP reset left the heavy `ps2fs`/`fileXio` modules alive in IOP RAM, instantly crashing POPSTARTER's specialized startup sequence.
    - a later 2026-03-31 hardware re-test restoring `SifIopReset` in the embedded loader resulted in another black screen. Code review revealed the EE-side DMA was hanging on `SifExitRpc()` because wiping the IOP completely destroyed the RPC command servers before teardown commands could be acknowledged.
    - current source completely removes the internal `SifIopReset` from the embedded loader (`loader.c`). This gracefully mirrors `wLaunchELF`'s true execution flow, maintaining the exact active IOP state (and active `pfs0:` mount) that `fileXio` used to successfully read the ELF, without hanging the EE's RPC teardowns.
  ...
  - `U-10`: one artifact was reported good before a later regression experiment.
    ...
    - current source now keeps the no-launch-CWD rollback and standard external-launch prep, retries `reboot_iop = 1` for BOOT.ELF after HDD init, and restores the generic reboot path in `src/elf_loader/src/elf.c` to the repo's older embedded-loader handoff style after the post-reset cleanup/module-reload contract from `src/system.cpp`.
    - current-source hardware result on that standard-prep/conditional-reboot line is still `Unknown (verify on hardware)`.
