...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - current source also returns the actual embedded-loader `ExecPS2` result instead of collapsing it to `-1`, fixes `System.loadELF(path, reboot_iop, args...)` so it forwards all extra args instead of dropping everything after the first one, now shows the actual exec path separately from the probed/opened POPSTARTER path in the launcher popup, routes partition-aware HDD launches back through mounted-`pfs0:` `SifLoadElf` in the embedded loader, normalizes stale canonical profile-path state so Profile 1/default does not silently keep another profile's HDD POPSTARTER path, keeps caller-supplied POPSTARTER selector/extra args intended to match the repo's normal non-HDD POPSTARTER argv layout, and keeps the older iomanX-aware `fileXio` path only as the fallback for direct `pfs:` / `hdd:` loads with no HDD partition context.
    ...
    - hardware result on the 2026-05-19 source remains `Unknown (verify on hardware)`. Verify `D-15` first to confirm non-HDD-POPSTARTER HDD-game still passes, then `D-10` `X`, then `D-10` `R2`, then `D-14`.
