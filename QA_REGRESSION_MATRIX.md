...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - audit also found a Lua binding bug in `src/luasystem.cpp`: the trailing reboot partition context was only recognized when `System.loadELF(...)` had at least four Lua arguments, so the zero-arg HDD path could not actually pass loader-only partition metadata until current source widened that check to the three-argument form.
    ...
    - clarification: POPSTARTER itself is not believed to require slot preservation, launch CWD, partition context, or carried runtime state after exec. The current temporary goal is only to get the HDD-backed ELF started at all; selector/`argv[0]` restoration is deferred. The loader may still need partition metadata to load the HDD ELF cleanly.
