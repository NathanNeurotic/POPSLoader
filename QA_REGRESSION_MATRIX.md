...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - current source now explicitly restores `SifIopReset("")` and the subsequent `rom0:` module reloads *after* `load_elf_via_filexio` succeeds, securely dropping the `pfs0:` mount and supplying POPSTARTER the pristine IOP state it expects.
