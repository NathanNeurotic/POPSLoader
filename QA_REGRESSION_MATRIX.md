...
| 2026-05-27 | PR #466 Hardware | BETA-12-PLAY post-merge. Tested: POPSLoader on USB launched from wLaunchELF. | DKWDRV-MC / Settings save USB+MC / BOOT.ELF-from-wLE-USB | (a) DKWDRV.ELF launch OK from default MC path (Nuno). (b) Settings save on USB and MC: PASS (Nuno) -- confirms sidecar works on non-HDD devices. (c) **NEW: BOOT.ELF exit when POPSLoader was launched via wLaunchELF FAILS** (Nuno) -- black screen. Same failure family as U-10 (upstream-parent-pollutes-state). Not previously tested explicitly; added to known-broken list. (d) MX4SIO / MMCE untested (no hardware on hand). |
