...
| 2026-03-30 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game with fileXioInit restored and .ELF extension | D-10 | Unknown (verify on hardware) |
...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - current source now properly retains `fileXioInit` to ensure `fileXioOpen` can utilize the preserved `pfs0:` mount.
