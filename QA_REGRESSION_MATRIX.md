...
| 2026-04-02 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game on mounted fileXio-first child-loader line (`0565ae5`) | D-10 | FAIL: black screen |
...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - a no-reset-first child-loader line for partition-aware HDD POPSTARTER (mounted fileXio-first + mounted `SifLoadElf` fallback, then direct `ExecPS2`) still black-screened on hardware.
    - current source now takes the bounded fallback iteration for this child path: mount `pfs0:`, load via mounted-path `SifLoadElf`, then apply child-side `SifIopReset("")` + `SIO2MAN`/`MCMAN`/`MCSERV` reload before `ExecPS2`. Hardware on this exact line is still `Unknown (verify on hardware)`.
    - latest user hardware report from the 2026-04-02 GitHub artifact (`cd76569`) still black-screened on `D-10`.
    - latest user hardware report from the 2026-04-02 GitHub artifact after that mounted fileXio-first child-loader line (`0565ae5`) also still black-screened on `D-10`.
  ...
  - `D-14`: reported FAIL on 2026-03-27 when launching a USB game with Profile 2 pointing `POPSTARTER.ELF` to HDD.
    ...
    - a later 2026-03-28 re-test on the direct-`hdd0:PART:pfsN:/POPSTARTER.ELF` preference source still black-screened on `X`.
    - current source now uses the same partition-aware HDD reboot contract as `D-10`, including the bounded fallback child-loader sequence (`pfs0:` remount, mounted-path `SifLoadElf`, child-side reset/module reload, then `ExecPS2`), rather than the earlier ad hoc source-context handoff or whichever mounted `pfsN:` path Lua happened to resolve first; hardware result is still `Unknown (verify on hardware)`.
