...
| 2026-03-29 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game on memory-wipe constrained and SifExitCmd omitted embedded-loader source | D-10 | Unknown (expected PASS: resolves previous runtime violations in `loader.c`) |
...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - it was later discovered that the embedded loader `loader.c` was incorrectly wiping the upper 1MB of system memory (corrupting RPC buffers/kernel structures) and calling `SifExitCmd()` immediately after `SifIopReset()`.
    - current source now restricts the `wipeUserMem` to `GetMemorySize() - 0x100000` to preserve necessary upper memory structures and skips the incorrect `SifExitCmd()` in the child loader post-IOP reset, matching `wLaunchELF` behavior.
    - current source also keeps the safer embedded-loader fix that avoids `printf`/`snprintf` in that environment, returns the actual embedded-loader `ExecPS2` result instead of collapsing it to `-1`, fixes `System.loadELF(path, reboot_iop, args...)` so it forwards all extra args instead of dropping everything after the first one, now shows the actual exec path separately from the probed/opened POPSTARTER path in the launcher popup, routes partition-aware HDD launches back through mounted-`pfs0:` `SifLoadElf` in the embedded loader, normalizes stale canonical profile-path state so Profile 1/default does not silently keep another profile's HDD POPSTARTER path, keeps caller-supplied POPSTARTER selector/extra args intact on that path so it matches the repo's normal non-HDD POPSTARTER argv layout, and keeps the older iomanX-aware `fileXio` path only as the fallback for direct `pfs:` / `hdd:` loads with no HDD partition context.
    - hardware result is now `Unknown (expected PASS: verification required on hardware)` since the direct cause of the execution hang has been eliminated.
  ...
  - `D-14`: reported FAIL on 2026-03-27 when launching a USB game with Profile 2 pointing `POPSTARTER.ELF` to HDD.
    ...
    - a later 2026-03-28 re-test on the direct-`hdd0:PART:pfsN:/POPSTARTER.ELF` preference source still black-screened on `X`.
    - current source now shares the `D-10` embedded loader fixes (memory wipe boundaries and SIF command exit fixes). Hardware result is `Unknown (expected PASS: verification required on hardware)`.
