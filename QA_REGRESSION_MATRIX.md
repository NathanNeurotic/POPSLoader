...
| 2026-03-30 | Unknown (not reported) | HDD boot; HDD sidecar/cwd `POPSTARTER.ELF`; HDD game with fileXioInit restored and .ELF extension | D-10 | FAIL: OSDSYS fallback (Parent loader never explicitly mounted pfs0:) |
...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - a later 2026-03-30 hardware re-test restoring `fileXioInit()` STILL returned to OSDSYS. Deep code review then exposed that the parent loader (`elf.c`) was generating a `keep_mask` of `1` (which skips unmounting `pfs0:`), but it never explicitly called `fileXioMount` to map the target partition to `pfs0:` in the first place! The earlier refactor removed the child loader's mount operation but failed to transplant it to the parent, leaving `pfs0:` effectively empty or unbound.
    - current source now properly retains `fileXioInit` and ensures the parent loader explicitly mounts the partition to `pfs0:` prior to execution handoff.
