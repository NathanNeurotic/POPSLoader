...
- Reported hardware outcomes:
  ...
  - `D-10`: reported FAIL when booted from HDD and launching an HDD title with HDD `POPSTARTER.ELF` sidecar/CWD.
    ...
    - current source now explicitly invokes `unmount_pfs_slots_for_exec` *before* executing the `fileXioMount` inside `build_hdd_embedded_loader_target_from_partition`, guaranteeing the target partition lock is freed before it is re-mapped to `pfs0:`.
