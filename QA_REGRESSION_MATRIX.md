...
| 2026-03-24 | Unknown | Standard artifact + HDD cwd + HDD-resident `POPSTARTER.ELF` + HDD game | D-10 | FAIL: commit `0a0b6e9` still flashed text very quickly and then black-screened, even after promoting the partitioned embedded-loader SIF-RPC keepalive into the normal path and disabling default diagnostic halt macros for plain builds; this did not produce a new durable stage and does not justify a new code target by itself; see `FAILURES.md` |
...
- Current standard-build status: commit `0a0b6e9` also still collapsed to flash-then-black for the HDD `cwd` / HDD-resident `POPSTARTER.ELF` case, so the branch remains unresolved without a newly justified code target.
