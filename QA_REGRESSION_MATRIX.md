...
- Reported hardware outcomes:
  ...
  - `D-14`: reported FAIL on 2026-03-27 when launching a USB game with Profile 2 pointing `POPSTARTER.ELF` to HDD.
    ...
    - a later 2026-03-28 re-test on the direct-`hdd0:PART:pfsN:/POPSTARTER.ELF` preference source still black-screened on `X`.
    - current source now uses the same mounted-`pfs0:` embedded-loader handoff as `D-10`, stops tearing down EE-side SIF state before `ExecPS2` into the embedded loader, no longer preserves boot PFS slots during launch prep for HDD-backed `POPSTARTER.ELF`, no longer unconditionally resets IOP inside the embedded loader before `ExecPS2`, and fixes the malformed direct-HDD alias builder; hardware result is still `Unknown (verify on hardware)`.
