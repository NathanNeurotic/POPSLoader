...
- The latest 2026-03-28 `D-10` run on that partition-aware source no longer black-screened, but the launcher regained control with `rc=-1 (returned after 3528 ms)`, which narrows the remaining failure to the parent `ExecPS2` jump into the embedded loader.
