...
- Current repo line now uses the legacy selector handoff as the default for HDD-backed POPSTARTER too: keep the resolved executable path, avoid Lua partition-context/generic-`pfs:/` rewriting on normal `X`, and preserve the partition-aware code only as a non-default fallback surface until hardware proves it is needed.
