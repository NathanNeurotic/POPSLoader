...
- Current repo line uses the partition-aware HDD reboot contract, separate exec-path reporting, profile-path normalization, and the bounded child-loader fallback handoff for partition-aware HDD POPSTARTER loads while preserving the restored non-HDD POPSTARTER path.
...
- A no-reset-first child-loader line (mounted fileXio-first with mounted `SifLoadElf` fallback) still black-screened on hardware.
- Current source now takes the bounded fallback iteration for partition-aware HDD POPSTARTER in the child loader: mount `pfs0:`, load via mounted-path `SifLoadElf`, then apply child-side `SifIopReset(\"\")` + `SIO2MAN`/`MCMAN`/`MCSERV` reload before `ExecPS2`; hardware on this exact line is still `Unknown (verify on hardware)`.
- Latest user hardware report from the 2026-04-02 GitHub artifact (`cd76569`) still black-screened on `D-10`.
- Latest user hardware report from the 2026-04-02 GitHub artifact after the mounted fileXio-first child-loader line (`0565ae5`) also still black-screened on `D-10`.
...
- Reproduce and resolve `D-10`:
  ...
  - current bounded fallback iteration keeps selector-only `argv[0]` and loader partition metadata, and now uses the partition-aware child-loader sequence `mount pfs0 -> SifLoadElf -> SifIopReset(\"\") + SIO2MAN/MCMAN/MCSERV reload -> ExecPS2`.
