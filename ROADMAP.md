...
- Current repo line now narrows the HDD-backed POPSTARTER experiment further: keep zero selector/extra args and just try to execute the resolved HDD ELF, while allowing only the minimum loader metadata needed to avoid the child `fileXio` shortcut.
...
- Current repo line now removes that stale exec-path rewrite from the stripped HDD-backed POPSTARTER experiment so probe/open and exec use the same resolved HDD ELF path, retries that same stripped HDD ELF launch through reboot mode instead of non-reboot `LoadExecPS2`, and lets that zero-arg reboot path still enter the HDD embedded loader.
...
- Audit also found a Lua binding bug: the trailing reboot partition context was only recognized when `System.loadELF(...)` had at least four Lua arguments, so the zero-arg HDD path could not actually pass loader-only partition metadata. Current source now accepts that three-argument form as well.
