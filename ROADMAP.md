...
- Remaining BETA-13 eyeballs before the cut (none currently believed broken — all "awaiting a look"): the Overscan (CRT inset) render-inset on a real CRT; the `cover_default.png` + `cover_missing.png` overlay registration against the jewel-case window on **both** NTSC and PAL; the description right-stick scroll feel; the new **Cover/details folder** setting (does POPS/ART vs POPS vs ART find removable art where expected, and does a `POPS/ART` cover still show now that the cover loader skips the `open()` existence pre-probe); and the HDD **Proposal A** scan-slot steering (deliberate HW test — see #5 below).
