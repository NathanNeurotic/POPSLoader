...
- Keep current cover behavior stable. On **every** device the cover `<name>.png` (and its matching `<name>.txt` details sidecar, which rides the same candidate list) is sought in a `POPS/ART/` subfolder **first** — the canonical, device-uniform location — then **beside the `.VCD`** as back-compat for art that predates the `ART/` folder (`BuildCoverCandidates`, `83e81cf`). Removable: `<device>:/POPS/ART/<game>.png` then `<device>:/POPS/<game>.png`; HDD/PFS: `hdd0:__common/POPS/ART/<game>.png` (from the `__common` partition). Within each, the disc-marker-stripped name is tried first (one art/details file serves every disc of a multi-disc game), then the exact per-disc `<full-name>.png` for back-compat.
