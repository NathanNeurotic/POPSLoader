...
**How to tell you are running it:** open **Settings**, then **About**, and read the **Version** row: it says **v1.0.2-dev-EXP15**. One step further into **Credits**, the small grey line at the bottom left says **POPSLoader v1.0.2-dev-EXP15** (or an exact build stamp, if a `BUILD_INFO.txt` file sits next to your POPSLOADER.ELF). The check is simple: a version ending in **-EXP15** = this build; any other EXP number = an older experimental, please update; plain **v1.0.2-dev** with no EXP = the rolling test build; no version anywhere = the public 1.0.1 release or older.
...
## New in EXP15: the memory check learns from your photo

sAGA's EXP14 photos were exactly what we needed: the freeze reproduced WITH the memory check passing, and the reboot message correctly said "it froze at step 45". That combination taught us our check was measuring at the wrong moment: it ran before the driver was loaded into memory, and the driver itself takes room before the risky allocation happens. EXP15 raises the requirement to cover both.

**The one-boot test, again:** open the HDD (exFAT) page once.
- If you now get a polite message about IOP memory (with numbers), photograph it: that confirms the memory theory and the proper fix is already prepared.
- If it says [pre200k=ok] and STILL freezes at 45, photograph that: it rules memory out completely and the remaining two suspects (both inside the drive-probe code, both already mapped) take over.
- Either photo moves us forward; there is no wasted outcome.

Also still valuable: the same test with every USB device unplugged, and, from anyone on the experimental build, a photo of the Credits screen's boot line (Settings, About, Credits, bottom left) -- the experimental build's boot has been reported slower than rolling's and that line names the slowest step.
...
| **This build (EXP15)** | **about 1,320,000 bytes** |
...
Please say **which build** (v1.0.2-dev-EXP15, from Settings then About), **which device**, your **console model and region**, and what you did. A photo helps, especially for anything text related.

If it all just works, that is a valuable report too. "EXP15 fine on USB and HDD, text and covers look normal" is genuinely all we need.
