...
- **MMCE and MX4SIO are gated: the first one you use owns the session.** This comes straight from R3Z3N (the wLE-R3Z author): the two adapters *"tie /ACK — a pin on the memcard port — differently"*, so running both drivers together is an **electrical** problem no software can arbitrate. wLE-R3Z switches between them with a full console-side reset; we do the equivalent — using the second one in a session shows *"restart to switch device"* instead of risking the bus. (An earlier plan to run them together was scrapped for this reason before it ever reached you.)
...
- **FifthFox (SCPH-70012, MMCE + MX4SIO both installed, back on the 25th):** update to EXP32, open the MX4SIO page with both adapters in — **on a fresh boot, before touching the MMCE page**. Expected: your games list within a few seconds. Then reboot and give MMCE its own pass (list + controllers + memory-card saves). One deliberate change to know about: using MMCE and MX4SIO **in the same session** now shows *"restart to switch device"* — that's the electrical /ACK gate the wLE-R3Z author told us both adapters need, not a bug. One device per boot, same as wLE-R3Z.
