...
static int read_full(int fd, void *buf, size_t size) {
    /*
     * Some IOP backends (notably PFS/HDD stacks depending on how we were launched)
     * can misbehave when reading directly into arbitrary destination pointers.
     * Read into an aligned bounce buffer and memcpy into the caller buffer.
     * This keeps behavior identical for working paths while avoiding "open ok, first read fails"
     * failures during POPSTARTER handoff.
     */
    uint8_t *dst = (uint8_t *)buf;
    size_t total = 0;

    /* 64-byte aligned bounce buffer (DMA-friendly). */
    ...
    while (total < size) {
        ...
        ssize_t rc = read(fd, bounce, want);
        if (rc <= 0) {
            return -1;
        }

        memcpy(dst + total, bounce, (size_t)rc);
        total += (size_t)rc;
    }
    return 0;
}
