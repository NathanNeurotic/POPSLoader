...
- **Per-game info text** — drop a `<game>.txt` next to a game, then turn on *Settings → Game details* (**Off / Left / Center / Right** alignment) to show it in a panel under the cover. Your line breaks are kept. Scroll long text with the **right analog stick** (fixed **Fast** pace, ~7 lines/sec — the Fast/Medium/Slow setting was removed since Fast tested best).
